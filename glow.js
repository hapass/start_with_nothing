...
var engine_graphics__$Renderer_QuadDrawingProgram = function(context,compiler,gameWidth,gameHeight) {
	...
	this.rand = this.context.getUniformLocation(this.program,"rand");
...
engine_graphics__$Renderer_QuadDrawingProgram.prototype = {
	...
	,drawQuads: function(quadArray,glowIndex) {
		if(engine_input_Key.SPACE.currentState == engine_input_Key.KEY_DOWN && engine_input_Key.SPACE.previousState == engine_input_Key.KEY_UP) {
			this.glowRadius = game_Config.GLOW_LIGHT_MIN_RADIUS;
			this.glowRadiusSpeed = game_Config.GLOW_LIGHT_STARTING_SPEED;
			this.animatingGlow = true;
			this.context.uniform3f(this.rand,Math.random(),Math.random(),Math.random());
		}
		if(this.animatingGlow) {
			this.glowRadiusSpeed += game_Config.GLOW_LIGHT_ACCELERATION;
			this.glowRadius += this.glowRadiusSpeed;
			if(this.glowRadius > game_Config.GLOW_LIGHT_MAX_RADIUS) {
				this.glowRadius = 0.0;
				this.animatingGlow = false;
			}
		}
...
var game_Config = $hx_exports["game"]["Config"] = function() { };
...
var game_Glow = function() {
	...
	this.shape.width = game_Config.GLOW_WIDTH;
	this.shape.height = game_Config.GLOW_HEIGHT;
...
game_Glow.prototype = {
	...
	,setPosition: function(x,y) {
		...
		this.topRightCornerCell.set(this.getColumn(this.position.x + game_Config.GLOW_WIDTH - 1),this.getRow(this.position.y));
		this.bottomLeftCornerCell.set(this.getColumn(this.position.x),this.getRow(this.position.y + game_Config.GLOW_HEIGHT - 1));
		this.bottomRightCornerCell.set(this.getColumn(this.position.x + game_Config.GLOW_WIDTH - 1),this.getRow(this.position.y + game_Config.GLOW_HEIGHT - 1));
	...
	,getColumn: function(x) {
		return Math.floor(x / game_Config.BRUSH_WIDTH);
	}
	,getRow: function(y) {
		return Math.floor(y / game_Config.BRUSH_HEIGHT);
	}
};
	...
	while(_g21 < _g3) {
		...
		while(_g22 < _g31) {
			...
			if(this.data[rowIndex][columnIndex] == 1 || this.data[rowIndex][columnIndex] == 2) {
				...
				rect.width = game_Config.BRUSH_WIDTH;
				rect.height = game_Config.BRUSH_HEIGHT;
			...
			if(this.data[rowIndex][columnIndex] == 3) {
				this.glowPosition.set(columnIndex * game_Config.BRUSH_WIDTH,rowIndex * game_Config.BRUSH_HEIGHT);
			}
		}
	}
};
...
var game_Game = function() {
	...
	this.renderer = new engine_graphics_Renderer(game_Config.GAME_WIDTH,game_Config.GAME_HEIGHT);
	this.keyboard = new engine_input_Keyboard([engine_input_Key.SPACE,engine_input_Key.RIGHT,engine_input_Key.LEFT,engine_input_Key.SHIFT]);
...
game_Game.prototype = {
	...
	,update: function(timestamp) {
		...
		this.glow.currentSpeed.add(0,game_Config.GLOW_FALL_ACCELERATION);
		if(engine_input_Key.RIGHT.currentState == engine_input_Key.KEY_DOWN) {
			this.glow.currentSpeed.set(game_Config.GLOW_SPEED,this.glow.currentSpeed.y);
		...
		if(engine_input_Key.SPACE.currentState == engine_input_Key.KEY_DOWN && engine_input_Key.SPACE.previousState == engine_input_Key.KEY_UP) {
			this.glow.currentSpeed.add(0,game_Config.GLOW_JUMP_ACCELERATION);
		}
			...
			if(this.level.getCellType(cell) == 1) {
				if(isHorizontal) {
					if(cell.x > previousCell.x && this.glow.isRight(cell) || cell.x < previousCell.x && this.glow.isLeft(cell)) {
						this.glow.setPosition(previousCell.x * game_Config.BRUSH_WIDTH,this.glow.position.y);
					}
					...
					this.glow.setPosition(this.glow.position.x,previousCell.y * game_Config.BRUSH_HEIGHT);
				}
...
engine_input_Key.SHIFT = new engine_input_Key(16);
...
game_Config.GAME_WIDTH = game_Config.GAME_WIDTH_BRUSHES * game_Config.BRUSH_WIDTH;
game_Config.GAME_HEIGHT = game_Config.GAME_HEIGHT_BRUSHES * game_Config.BRUSH_HEIGHT;
...
game_Config.GLOW_WIDTH = game_Config.GLOW_WIDTH_BRUSHES * game_Config.BRUSH_WIDTH;
game_Config.GLOW_HEIGHT = game_Config.GLOW_HEIGHT_BRUSHES * game_Config.BRUSH_HEIGHT;
game_Config.GLOW_LIGHT_MIN_RADIUS = 0.015;
game_Config.GLOW_LIGHT_MAX_RADIUS = 0.2;
game_Config.GLOW_LIGHT_STARTING_SPEED = 0.0008;
game_Config.GLOW_LIGHT_ACCELERATION = 0.0007;
